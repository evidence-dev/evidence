...
WITH EVERYTHING AS (
    SELECT  COUNT(l.id) as likes,
        ...
        WHERE p.created_at > (CURRENT_DATE - INTERVAL '${inputs.time_range.value}S')
...
SELECT COUNT(*) as postCount FROM posts p
    INNER JOIN post_tags pt on p.id = pt.post_id
WHERE p.created_at > CURRENT_DATE - INTERVAL '${inputs.time_range.value}' AND pt.hashtag_id = ${inputs.selected_tag.value ?? -1}
...
WHERE pt.hashtag_id = ${inputs.selected_tag.value ?? -1} AND p.created_at > CURRENT_DATE - INTERVAL '${inputs.time_range.value}'
GROUP BY pt.hashtag_id
```
...
<div class="grid grid-cols-3">
    <BigValue data={total_posts} value="postCount" title="Posts with #{inputs.selected_tag.value}"/>
    <BigValue data={unique_post_authors} value="authorCount" title="Unique authors posting about #{inputs.selected_tag.value}"/>
    <BigValue data={unique_post_likers} value="authorCount" title="Unique users liking posts about #{inputs.selected_tag.value}"/>
</div>
<div class="grid grid-cols-3">
    <Value data={total_posts} value="postCount" title="Posts with #{inputs.selected_tag.value}"/>
    <Value data={unique_post_authors} value="authorCount" title="Unique authors posting about #{inputs.selected_tag.value}"/>
    <Value data={unique_post_likers} value="authorCount" title="Unique users liking posts about #{inputs.selected_tag.value}"/>
</div>
{/if}
