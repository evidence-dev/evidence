      ...
      <h2 class="font-ui font-bold text-lg">
        <button 
          ...
          <span class="transform {index === openIndex ? 'rotate-180' : ''}">
						<svg viewBox="0 0 20 20" style="height: 20px; width:20px" class="stroke-current" fill="none">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
            </svg>
          </span>
        </button>
      </h2>
      <div class="text-base overflow-hidden transition-all duration-300 max-h-0 {index === openIndex ? 'max-h-80' : ''}">
        <div class="p-5">
          <p>{item.content}</p>
        </div>
      </div>
