...
function gcd(a, b) // Get greatest common divisor of the differences between values
{
  if (a < b)
    return gcd(b, a);
        ...
        if(fillX){
            ...
            if(nullsZero){
                filledData = tidy(
                    data, 
                    complete(
                        {[x]: vectorSeq(xDistinct, interval), [series]: series},
                        {[y]: 0}
                    )
                )    
            } else {
        ...
        if(fillX){
            ...
            if(nullsZero){
                filledData = tidy(
                    data, 
                    complete(
                        {[x]: vectorSeq(xDistinct, interval)},
                        {[y]: 0}
                    )
                )    
            } else {
