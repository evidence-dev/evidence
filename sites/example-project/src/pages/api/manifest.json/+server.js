...
export async function GET() {
    ...
    async function getDirPages(dirPath) {
        ...
        const output = {}
        if (!dirPath.includes("queries") && !dirContent.some(d => d.name === "queries")) return output
        ...
        if (dirContent.some(d => d.name === "+page.md")) {
            ...
            const preprocessedContent = await sveltePreprocess(fileContent, preprocess(), { filename: pageFilepath }).then(r => `\n${r.code}`)
