...
const loadDB = async () => {
	...
	if (!browser) {
		...
		const res = await fetch(buildUrl('/data/manifest.json'));
...
async function getPrerenderedQueries(routeHash, paramsHash, fetch) {
	...
	const res = await fetch(buildUrl(`/api/${routeHash}/${paramsHash}/all-queries.json`));
	if (!res.ok) return {};
	...
	const resolved_entries = await Promise.all(
		Object.entries(sql_cache_with_hashed_query_strings).map(async ([query_name, query_hash]) => {
			const res = await fetch(buildUrl(`/api/prerendered_queries/${query_hash}.arrow`));
...
export const load = async ({ fetch, route, params, url }) => {
	const [{ customFormattingSettings }, pagesManifest, evidencemeta] = await Promise.all([
		fetch(buildUrl('/api/customFormattingSettings.json/GET.json')).then((x) => x.json()),
		fetch(buildUrl('/api/pagesManifest.json')).then((x) => x.json()),
		fetch(buildUrl(`/api/${route.id}/evidencemeta.json`))
			.then((x) => x.json())
			.catch(() => ({ queries: [] }))
	]);
