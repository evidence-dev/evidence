...
<ComboChart {...comboProps} tagName="area_chart">
	<MultiSeries {...comboProps}>
		{#snippet seriesSnippet(multiSeriesProps)}
			<!--
				echarts_options={undefined}: area_chart's echarts_options is chart-wide (from
				the inherited combo_chart schema); the child <Area> also has its own per-series
			...
			<Area
				...
				metric={undefined}
				{stacked}
				options={comboProps.area_options}
