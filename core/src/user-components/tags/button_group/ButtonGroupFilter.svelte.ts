...
type ButtonGroupAttributes = UserComponentProps<typeof schema> & {
	// Options list passed from the component for label lookup
	_combinedOptions?: Option[];
};

export class ButtonGroupFilter extends Filter<string | string[]> {
	attributes: Omit<ButtonGroupAttributes, 'id'>;
	...
	get templateValues() {
		// Generate template values for button group
	...
	constructor(init: FilterInit<'button_group', ButtonGroupAttributes>, deps: FilterDeps) {
