...
export interface ValueSQLAttrs extends BaseSQLAttrs {
	value: string;
...
export function buildValueSQLConfig(attrs: ValueSQLAttrs): SQLQueryConfig {
...
export function buildValueSQL(attrs: ValueSQLAttrs): SQLQueryResult {
	const firstDayOfWeek = attrs.firstDayOfWeek ?? 'sunday';
	const dialect = attrs.dialect ?? defaultDialect;
	const config = buildValueSQLConfig(attrs);
