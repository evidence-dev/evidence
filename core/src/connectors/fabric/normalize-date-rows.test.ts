...
describe('normalizeDateRows (Fabric / SQL Server)', () => {
	...
	it('preserves sub-second precision by NOT collapsing 00:00:00.123 to a date', () => {
		// T-SQL DATETIME2/DATETIMEOFFSET keep ms; tedious surfaces them in the JS
		// Date. If we collapsed on hour/minute/second alone we'd silently drop the
		// fractional second.
	...
	it('leaves non-date columns alone', () => {
		const rows = [{ d: new Date('2024-01-01T00:00:00.000Z'), name: 'x' }];
		normalizeDateRows(rows, new Set(['d']));
