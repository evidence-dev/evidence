...
const hasQueries = function(filename){
    ...
    return fs.existsSync("./queries/extracted/"+hash)
}
...
const updateExtractedQueriesDir = function(content, filename){
    if (!fs.existsSync("./queries")){
        fs.mkdirSync("./queries");
    }
    if (!fs.existsSync("./queries/extracted")){
        fs.mkdirSync("./queries/extracted");
    }
    let routeHash = getRouteHash(filename)
    let queryDir = `./queries/extracted/${routeHash}`
