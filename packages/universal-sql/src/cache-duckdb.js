...
export function cache_for_hash(
	...
	mkdirSync(`${cache_path}/${additional_hash}`, { recursive: true });
	writeFileSync(`${cache_path}/${additional_hash}/${query_name}.arrow`, tableToIPC(data));
	...
	if (prerendering) {
		...
		mkdirSync(`${prerender_path}/${additional_hash}`, { recursive: true });
		writeFileSync(`${prerender_path}/${additional_hash}/${query_name}.arrow`, tableToIPC(data));
	}
}
...
export function get_cache_for_hash(route_hash, additional_hash, query_name) {
	const cache_path = `./.evidence-queries/cache/${route_hash}/${additional_hash}`;
