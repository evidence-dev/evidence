		...
		{#each orderedColumns as column, k}
			...
			>
				...
				{:else if column.contentType === 'bar' && row[column.id] !== undefined}
							{@const total_width = column_min >= 0 ? column_max : column_max < 0 ? column_min : Math.abs(column_min) + column_max}
					<div style="width: 100%; background-color: {column.backgroundColor}; position: relative; height: 100%; display: flex; align-items: center; overflow: hidden;">
						...
						{#if row[column.id] < 0}
							<div
							style="width: {Math.min(Math.abs((row[column.id] / total_width) * 100), 100)}%; background-color: {column.negativeBarColor}; height: 100%; position: absolute; right: {(1 - (Math.abs(column_min) / (Math.abs(column_min) + column_max))) * 100}%;"
							></div>
						{/if}
						{/if}
						...
						{#if row[column.id] >= 0}
						<div
							style="width: {Math.min((row[column.id] / total_width) * 100, 100)}%; background-color: {column.barColor}; height: 100%; position: absolute; left: {column_min < 0 ? (Math.abs(column_min) / (Math.abs(column_min) + column_max)) * 100 + '%' : '0'};"
						></div>
						{/if}
					
						<!-- Display label -->
						<div class:invisible={column.hideLabels} style="position: relative; z-index: 2; padding-left: 4px; padding-right: 4px; text-align: {column.align ?? 'left'}; width: 100%;">
