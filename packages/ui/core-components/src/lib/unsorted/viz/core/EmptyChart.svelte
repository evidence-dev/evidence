...
{#if ['warn', 'pass'].includes(emptySet) || !isInitial}
	{#if chartType === 'Value'}
		<span
			class="text-xs text-base-content/70 p-2 my-2 w-full border border-base-300 border-dashed rounded"
			>{emptyMessage}</span
		>
	{:else if chartType === 'Big Value'}
		<p
			class="text-xs text-base-content/70 p-2 pt-[32px] my-0 text-center w-full align-middle h-[80px] border border-base-300 border-dashed rounded min-w-[120px]"
		>
			{emptyMessage}
		</p>
	{:else}
