...
<RadioGroup.Root bind:value={selectedMetric} type="single" asChild let:builder>
	<div
		class="text-xs text-gray-500 grid grid-cols-4 gap-x-2 tabular-nums"
		use:builder.action
		{...builder}
	>
		{#each $metricStore as metric}
			<RadioGroup.Item value={metric.label} asChild let:builder>
				<div
					class="cursor-pointer grid-cols-subgrid grid col-span-6 relative items-center p-2 rounded-md group"
					use:builder.action
					{...builder}
				>
					<div
