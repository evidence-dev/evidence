...
export const filter = (rows, filters) => {
    ...
    for (const filter of filters) {
        switch (filter.type) {
            case 'unique':
                // O(n) unique function
                console.debug(`Running unique filter with ${filter.fields.length} fields over ${rows.length} rows. This may take some time`)
                 ...
                 */
                const buildKey = (r) =>
                    JSON.stringify(filter.fields.map(f => r[f]))
