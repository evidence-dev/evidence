...
const getFakerValue = (category, item, options, targetField, colName) => {
    if (!(category in faker)) throw new Error(`(${colName}) ${category} is not a valid category; please see https://fakerjs.dev/api/`)
    if (!(item in faker[category])) throw new Error(`(${colName}) ${category}.${item} does not exist; please see https://fakerjs.dev/api/`)
...
const buildRow = async (schema, tableName, rowNum) => {
    ...
    for (const colName in schema) {
        if (schema[colName].type === "id") {
            ...
            if (!targetTablePath) {
                throw new Error(`Failed to get table path for ${colName}. This is a bug in faker-datasource`)
            }
            ...
            if (!(targetField in targetRow)) {
                throw new Error(`Target (${schema[colName].target?.table}) does not contain field ${targetField}`)
            }
...
const generateTable = (directory) => async (content, filepath) => {
    ...
    try {
        ...
        if (!("rows" in definition)) throw new Error(`${filepath.split("/").pop()} is missing required required field "rows"`)
        if (!(typeof definition.rows === "number")) throw new Error(`${filepath.split("/").pop()} "rows" must be a number`)
        if (!("schema" in definition)) throw new Error(`${filepath.split("/").pop()} is missing required required field "schema"`)
        if (!(typeof definition.schema === "object")) throw new Error(`${filepath.split("/").pop()} "schema" must be an object`)
        ...
        const foreignKeyTargets = foreignKeys.map((col) => {
        ...
        for (const fkt of foreignKeyTargets) {
            ...
            if (relations.has(fkt) && relations.get(fkt).includes(filepath)) {
                throw new Error(`Circular dependency found in foreign keys; ${filepath} -> ${fkt} -> ${filepath}`)
            }
