...
// prettier-ignore
const disclaimer = `
${bold(green('Welcome to Evidence!'))}

${bold(red('This is alpha software; expect bugs and missing features.'))}

If you encounter a problem, we're here to help. Open an issue on ${cyan('https://github.com/evidence-dev/evidence')}.
`;

const { version } = JSON.parse(fs.readFileSync(new URL('package.json', import.meta.url), 'utf-8'));

async function main() {
	console.log(gray(`\ncreate-evidence version ${version}`));
	...
	write_template_files(options.template, false, name, cwd);
	...
	console.log(
		`\nStuck? Want to chat? Join us on Slack ${cyan('https://docs.evidence.dev/community')}\n`
	);
}
