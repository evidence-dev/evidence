		...
		{#if j === 0}
			<td class="font-medium" colspan={rowNumbers ? 2 : 1} style="padding: 3px;">
				<div class="items-center gap-2 align-top">
					<span class="inline-flex print-hidden chev"><TableChevronToggle {toggled} /></span>
					{groupName}
				</div>
			</td>
		{:else}
			<td class="{columnSummary[j].type} font-medium">
				{#if column.contentType === 'delta'}
					...
					/>
				{:else}
					{formatValue(currentGroup[column.id], format, useCol.columnUnitSummary)}
				{/if}
			</td>
		{/if}
	{/each}
