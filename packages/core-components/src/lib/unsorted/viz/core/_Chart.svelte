<script>
	...
	export let seriesSortOrder = 'asc';
	...
	$: {
		try {
			...
			// ---------------------------------------------------------------------------------------
			// Sort data based on xType, sort prop, and seriesSort prop
			// ---------------------------------------------------------------------------------------
			data = sort
				? xDataType === 'category'
			...
			// If a seriesSort column is supplied, sort the data by that column
			if(seriesSort) {
				data = getSortedData(data, seriesSort, seriesSortOrder === 'desc'? false : true);
			}
