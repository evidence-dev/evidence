...
const duckdbKeywords = [
    'ANY', 'ASC', 'BETWEEN', 
    'CROSS JOIN', 'DESC', 'DISTINCT', 
    'ELSE', 'END', 'EXISTS', 'EXTRACT', 'FALSE', 'FROM', 'FULL JOIN', 
    'GROUP BY', 'GROUP BY ALL', 'HAVING', 'ILIKE', 'IN', 'INNER JOIN', 'INTERSECT', 'IS', 'JOIN', 
    'LEFT JOIN', 'LIKE', 'LIMIT', 'NATURAL JOIN', 'NOT', 'NULL', 'OFFSET', 'ON', 'ORDER BY', 
    'OUTER JOIN', 'PARTITION BY', 'RIGHT JOIN', 'SELECT', 'SIMILAR TO', 'THEN', 
    'TRUE', 'UNION', 'UNION ALL', 'UNIQUE', 'USING', 'VALUES', 'WHEN', 'WHERE', 'WITH'
];
...
const duckdbFunctions = [
    ...
    },
	{
		...
		documentation: 'Extracts a substring starting at the start character and extending for characters supplied in length. Note that a start value of 1 refers to the first character of the string'
    },
	{
        name: 'STRPOS',
        detail: 'STRPOS(string, search_string)',
		snippet: "STRPOS(${1:string}, ${2:search_string})",
		documentation: 'Returns location of first occurrence of search_string in string, counting from 1. Returns 0 if no match found'
    },
	{
        name: 'STARTS_WITH',
    ...
    },
	{
        ...
        detail: 'FILTER (x)',
		snippet: "FILTER (${1:x})",
		documentation: 'Filters an aggregation for rows where x is true. x is a logical expression - e.g., FILTER (year = 2023)'
    },
	{
        name: 'IFNULL',
        detail: 'IFNULL(value, fallback_value)',
		snippet: "IFNULL(${1:value}, ${2:fallback_value})",
		documentation: 'Returns the first value if it is not null. Otherwise, returns the fallback_value'
    },
	{
        name: 'COALESCE',
    ...
    },
	{
		...
		documentation: 'Returns the value X rows below the current value, where x is determined by row_offset. A third argument can be supplied to indicate the value that should be inserted if the row does not exist - by default this is null'
    },
	{
        name: 'LEAD',
        detail: 'LEAD(column, row_offset) OVER (ORDER BY order_column)',
		snippet: "LEAD(${1:column}, ${2:row_offset}) OVER (ORDER BY ${3:order_column})",
		documentation: 'Returns the value X rows above the current value, where x is determined by row_offset. A third argument can be supplied to indicate the value that should be inserted if the row does not exist - by default this is null'
    },
	{
        name: 'ROW_NUMBER',
    ...
    },
	{
		...
		documentation: 'Returns the rank of the current row based on the values in the order_column. If 2 order_column values are identical, the rank will be the same'
    },
	{
        name: 'DENSE_RANK',
        detail: 'DENSE_RANK() OVER (ORDER BY order_column)',
		snippet: "DENSE_RANK() OVER (ORDER BY ${1:order_column})",
		documentation: 'Same as RANK(), but does not increment after a tie. Returns the rank of the current row based on the values in the order_column. If 2 order_column values are identical, the rank will be the same.'
    },
	{
        name: 'OVER',
        detail: 'OVER()',
		snippet: "OVER()",
		documentation: 'Used in window functions to determine which order or partitions to use for the window. Follows another function.'
    },
	{
        name: 'LEAST',
        detail: 'LEAST(value1, value2)',
		snippet: "LEAST(${1:value1}, ${2:value2})",
		documentation: 'Returns the lowest value of the provided values. Can supply more than 2 values if needed.'
    },
	{
        name: 'PIVOT',
...
async function updateEditorConfigForLanguage(languageId: string, isSQLContext: boolean) {
    ...
    try {
        if (isSQLContext) {
            await config.update('acceptSuggestionOnEnter', sqlAcceptSuggestionsOnEnter, ConfigurationTarget.Workspace);
            await config.update('quickSuggestions', { other: true, comments: false, strings: false }, ConfigurationTarget.Workspace);
			await config.update('quickSuggestionsDelay', 0, ConfigurationTarget.Workspace);
        } else {
            await config.update('acceptSuggestionOnEnter', slashCommandsAcceptSuggestionsOnEnter, ConfigurationTarget.Workspace);
            await config.update('quickSuggestions', { other: false, comments: false, strings: false }, ConfigurationTarget.Workspace);
...
function applyCaseToFunction(func: { name: string; detail: string; snippet: string; documentation: string; } | { name: string; detail: string; snippet: string; documentation?: undefined; }, setting: string) {
    return {
...
async function provideSQLCompletionItems(document: TextDocument, position: Position, context: string) {
    ...
    let keywords = [];
	let functions = duckdbFunctions.map(func => applyCaseToFunction(func, sqlKeywordSuggestionCase));

    keywords = applyCaseToKeywords(duckdbKeywords, sqlKeywordSuggestionCase);

    const keywordCompletionItems = keywords.map(keyword => new CompletionItem(keyword, CompletionItemKind.Keyword));
...
export async function activate(context: ExtensionContext) {
    ...
    window.onDidChangeTextEditorSelection(event => {
        if (window.activeTextEditor && event.textEditor.document === window.activeTextEditor.document) {
            applyCustomSettings();
        }
    });
