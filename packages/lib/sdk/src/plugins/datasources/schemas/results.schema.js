...
const QueryResultDataSchema = QueryResultMetaSchema.extend({
	// Note that this only validates that the first item in the array
	// is a record with string keys. If the connector returns some
	// inconsistent array (e.g. [{}, 1]), it will not detect the
	// invalid row.
...
const QueryResultUrlSchema = QueryResultMetaSchema.extend({
	url: z.string()
});

export const QueryResultSchema = QueryResultUrlSchema.or(QueryResultDataSchema);
