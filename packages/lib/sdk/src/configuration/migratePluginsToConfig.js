...
export const migratePluginsToConfig = async () => {
	...
	let config = EvidenceConfigSchema.parse({ plugins: {} });
	const legacyConfig = getEvidenceConfigLegacy(true);
	try {
		config = getEvidenceConfig(EvidenceConfigSchema, false);
	} catch {
		/* ignore errors */
	}

	config.plugins = merge(legacyConfig?.plugins ?? {}, config.plugins);
