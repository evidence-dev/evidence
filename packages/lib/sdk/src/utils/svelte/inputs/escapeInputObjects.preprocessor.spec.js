...
describe("escapeInputObjects", () => {
	...
	it('should an insertion on {inputs.something}', () => {
        expect(
            testStr('{inputs.something}')
        ).toBe(`{inputs.something${subStr}}`)
    })
	it('should an insertion on {inputs.something.somethingelse}', () => {
        expect(
            testStr('{inputs.something.somethingelse}')
        ).toBe(`{inputs.something.somethingelse${subStr}}`)
    })
    ...
    it('should not do an insertion on <Component attr={inputs.something} />', () => {
        expect(
            testStr('<Component attr={inputs.something} />')
        ).toBe('<Component attr={inputs.something} />')
    })
    it('should do an insertion on <Component attr="string interpolation {inputs.something}" />', () => {
        expect(
            testStr('<Component attr="string interpolation {inputs.something}" />')
        ).toBe(`<Component attr="string interpolation {inputs.something${subStr}}" />`)
    })
    it('should do an insertion on { inputs.something }', () => {
        expect(
            testStr('{ inputs.something }')
    ...
    it('should do an insertion on { inputs.something ? true : false }', () => {
        expect(
            testStr('{ inputs.something ? true : false }')
        ).toBe(`{ inputs.something${subStr} ? true : false }`)
    })
    it('should do an insertion on the first occurence of inputs in { inputs.something ? inputs.something.toLowerCase() : false }', () => {
        expect(
            testStr('{ inputs.something ? inputs.something.toLowerCase() : false }')
        ).toBe(`{ inputs.something${subStr} ? inputs.something.toLowerCase() : false }`)
    })

    it('should do nothing with a script tag', () => {
        expect(
            testStr('<script>inputs.something</script>')
        ).toBe('<script>inputs.something</script>')
    })
});
