...
A markdown file in `components/` is a component by default, so the `type: component` frontmatter below is optional there. An explicit `type:` always wins: set `type: page` or `type: partial` on a file in `components/` and it is excluded from component discovery, and add `type: component` to a file outside `components/` to colocate a component next to the page that uses it.
