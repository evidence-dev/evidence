...
`start` and `end` are SQL expressions for the active dialect — writing them in markdown prints `toDate('2026-07-01')`. Use `start_label` / `end_label` for text.
...
| `period.filter` | `order_date >= … AND order_date <= …` | Complete filter expression. Requires `value_column`. |
