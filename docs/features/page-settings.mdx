...
The picker follows the project's [date settings](/features/project-settings): "complete" is judged against `date.default_date_range_end` (so `days_ago: 1` shifts which period counts as finished, and a `custom_sql` end pins periods to the data actually loaded), and the `week` grain starts weeks on `date.first_day_of_week`.
