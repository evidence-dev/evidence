...
The **Evidence Agent** is Evidence's bi ai agent for your project — an artificial intelligence for business that lets your readers ask data questions as a natural language query and get analytical answers built from your project. It's one of the ai business intelligence tools that brings AI analytics to your published reports and dashboards: the agent reads them as context, generates new **insights** (analyses written in Evidence Markdown), and can reach the underlying SQL and filter definitions behind your reports. It respects your project's existing [access rules](/features/row-level-security) and [page-level access control](/features/page-level-access-control), so readers only ever see data they're allowed to — enabling self serve bi and self service business intelligence where readers answer their own questions without waiting on an analyst.
