...
description: 'Deliver secure, personalized customer facing dashboards inside your own product'
keywords:
  - embedded analytics
...
Embedded analytics lets you deliver personalized Evidence reports to your customers directly within your application. The business intelligence integration is built on a two-step authentication flow and an iframe embed, so each user sees only their data and the reports match your application's design. Because themes match your brand and published reports surface your organization's logo rather than Evidence branding, embedded analytics works well as white label analytics — letting you ship customer facing dashboards inside your own product.
...
- **Row Level Security**: Integrates with [row-level security rules](/features/row-level-security) to filter data by user
...
<Steps>
  ...
  <Step title="Create RLS Rules (Optional)">
    If you need to filter data based on user attributes, set up [row-level security rules](/features/row-level-security) in your [Access Settings](https://evidence.studio/settings/access-rules).
...
User attributes are used with [row-level security rules](/features/row-level-security) to filter data. Evidence supports rich attribute types:
...
4. **Apply RLS rules**: Use [row-level security](/features/row-level-security) to filter data appropriately
