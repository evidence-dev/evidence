...
Page level access control determines who can view a project's published reports — it's the permissions layer that governs access to your content. It is defined in a single `access.yaml` file at the project root, created for you automatically. The file lives alongside your content, ships through the same git workflow as your pages, and is enforced when readers visit your published reports. Because access is version-controlled and reviewed in pull requests, this gives you content governance over who sees what — a page level security model that acts as a content security policy for your reports. The model is RBAC as code: in `access.yaml` you grant view access to users, groups, and `$org`, and only those principals can view the pages you grant them.
...
<Note>
	**Pair page-level access with Row-Level Security for true data isolation between customers.**
	...
	[Row-Level Security](/features/row-level-security) decides which rows of data they
